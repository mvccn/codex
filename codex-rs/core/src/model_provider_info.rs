...
pub enum WireApi {
    ...
    /// Native Gemini streaming API. When using this wire protocol, the
    /// `base_url` is expected to point at the full Gemini streaming endpoint
    /// (for example, a `streamGenerateContent` REST URL), and Codex will use
    /// a dedicated adapter to translate between Gemini's JSON format and
    /// internal `ResponseEvent`s.
    Gemini,
}
